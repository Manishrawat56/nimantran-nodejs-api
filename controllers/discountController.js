const DiscountEntity = require("../models/discount");
const {
  validation,
...
exports.update = async function (req, res, next) {
  try {
    ...
    const storeBanner=await DiscountEntity.findById(entity.id);
    if(!storeBanner){
      sendNotFoundApiResponse(res,MSG.couldNotFindAnyItems)
    }
    
    const data = await DiscountEntity.findByIdAndUpdate(entity.id, entity);
    if (data) {
...
exports.getById = async function (req, res, next) {
  try {
    const data = await DiscountEntity.findById(req.params.discountId);
    if (data) {
...
exports.delete = async function (req, res, next) {
  try {
    const data = await DiscountEntity.findByIdAndRemove(req.params.discountId);
    if (data) {
...
exports.getAll = async function (req, res, next) {
  try {
    ...
    const data = await DiscountEntity.find();
    if (data) {
      return sendOkApiResponse(res, MSG.success, data);
    } else {
      internalServerError(MSG.somethingWentWrong);
...
exports.deleteAll = async function (req, res, next) {
  try {
    const data = await DiscountEntity.deleteMany({});
    if (data) {
      return sendOkApiResponse(res, MSG.success, data);
    } else {
